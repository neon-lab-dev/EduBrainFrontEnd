...
import API from ".";
import { type LoginFormData, type SignUpFormData } from "../components/reusable/AuthenticationModal/AuthenticationModal";
...
export const handleForget = async (forgetPasswordData : ForgetPassData) : Promise<unknown> => {
    try {
...
export const handleNewPassword = async ({data, token} : CreateNewPassword) : Promise<unknown> => {
    try {
