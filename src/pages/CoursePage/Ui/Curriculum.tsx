...
const Curriculum: React.FC = (): JSX.Element => {
                ...
                <div className='flex justify-between gap-1'>
                    ...
                    <div className='flex gap-10 max-sm:gap-1'>
                        <div className='bg-[#246BFD1A] rounded-xl'>
                            <span className='text-[#246BFD] text-[10px] p-4 cursor-pointer'>Preview</span>
                        </div>
                        <div>
    ...
    const refs: React.RefObject<HTMLDivElement>[] = Array.from({ length: numberOfItems }, () => useRef<HTMLDivElement | null>(null));
    ...
    const isActivePart = (index: number): string => {
        return activeIndex === index ? "border-[#91B5FE] text-[16px] text-[#91B5FE]" : "border-[#252526] text-[16px] text-[#7A7C80]";
    };

    const scrollIntoViewIfNeeded = (ref: React.RefObject<HTMLDivElement>): void => {
        ...
        <div className='py-16 p-2'>
            ...
            <div className='flex py-10 max-lg:pl-0 max-md:pl-0 max-lg:justify-center gap-8 max-sm:gap-2 '>
                <div className='flex pl-[110px] max-lg:pl-0 max-md:pl-0 max-lg:justify-center gap-8 max-sm:gap-2 '>
                    <div className={`border px-10 max-sm:px-3 py-2 rounded-xl cursor-pointer ${isActivePart(0)}`} onClick={() => { handleClick(0); }}>
                        <span className='text-[24px] max-sm:text-[14px] '>Part-1</span>
                    </div>
                    <div className={`border px-10 max-sm:px-3 py-2 rounded-xl cursor-pointer ${isActivePart(1)}`} onClick={() => { handleClick(1); }}>
                        <span className='text-[24px] max-sm:text-[14px]'>Part-2</span>
                    </div>
                    <div className={`border px-10 max-sm:px-3 py-2 rounded-xl cursor-pointer ${isActivePart(2)}`} onClick={() => { handleClick(2); }}>
                        <span className='text-[24px] max-sm:text-[14px]'>Part-3</span>
                    </div>
                    <div className={`border px-10 max-sm:px-3 py-2 rounded-xl cursor-pointer ${isActivePart(3)}`} onClick={() => { handleClick(3); }}>
                    ...
                    <div className=' flex flex-col gap-4 p-4 max-sm:p-0 max-xl:w-[400px]  w-[650px] max-lg:w-[740px] max-sm:w-[320px]'>
                        ...
                        <div className='text-white py-6 max-lg:hidden'>
                            <ul className='list-disc pl-8 text-[24px] flex flex-col gap-5 cursor-pointer'>
                                <li className={isActivePart(0)} onClick={() => { handleClick(0); }}>Color</li>
                                <li className={isActivePart(1)} onClick={() => { handleClick(1); }}>Typography</li>
                                <li className={isActivePart(2)} onClick={() => { handleClick(2); }}>Button</li>
                                <li className={isActivePart(3)} onClick={() => { handleClick(3); }}>Image</li>
                            </ul>
                        </div>
                    </div>
